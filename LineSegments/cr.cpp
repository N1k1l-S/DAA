...
float cr_area_of_outline ( vector <Point> &outline){
    float a = 0.00; 
    int n = outline.size();
    for ( int i = 0;  i < n-1; i++ ){
        a += 0.50*( outline[i+1].X() - outline[i].X())* (outline[i+1].Y() + outline[i].Y());
    }

    return a;
}

float cr_lengthCovered_of_outline ( vector <Point> &outline){
    float l = 0.00; auto start = outline.begin();
    for ( auto it = start; it != outline.end() ; it++  ){
        if ( next(it) != outline.end()){
            if ( next(it)->Y() + it ->Y() ){
                l += next(it)->X() - it->X() ;
            }
        }    
...
int main(){
	...
	float cr_lengthCovered = cr_lengthCovered_of_outline(merged_vector); 
	...
	float cr_area = cr_area_of_outline(merged_vector);
	...
	cout << merged_vector.size()<< endl;
	cout << static_cast<int>(lengthCovered)<< endl;
	cout << static_cast<int>(cr_lengthCovered)<< " cr len " << endl;
	
	cout << static_cast<int>(area)<< endl;
	cout << static_cast<int>(cr_area)<< " cr area " << endl;
